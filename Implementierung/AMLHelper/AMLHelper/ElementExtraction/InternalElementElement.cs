...
namespace AMLHelper.ElementExtraction
{
    ...
    public class InternalElementElement : CAEXElement
    {
        ...
        private void CreateTree(InternalElementType internalElement)
        {
            ...
            if (!internalElement.RefBaseSystemUnitPath.Exists()) return;

            string id = internalElement.RefBaseSystemUnitPath.Value;

            var sysclass = FileInstance.GetInstance().Data.getReferencedSystemUnitClass(id);
            if (sysclass == null) return; //Es existiert keine Sysclass.

            foreach (SupportedRoleClassType role in sysclass.SupportedRoleClass)
            {
                bool hasRole = false;
                foreach (SupportedRoleClassType srcType in internalElement.SupportedRoleClass)
                {
                    if (!srcType.RefRoleClassPath.Exists() || !role.RefRoleClassPath.Exists()) continue;
                    if (srcType.RefRoleClassPath.Value == role.RefRoleClassPath.Value)
                    {
                        hasRole = true;
                        break;
                    }
